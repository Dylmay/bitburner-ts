import { createNiceError, ErrorArg } from 'lib/utils/errors';
...
type NonGuardTypeArg = PrimitiveName | AnyConstructor;
...
function resolveTypeName(typeArg: NonGuardTypeArg): string {
...
function checkFieldDef(
  ...
  return [
    {
      path: [fieldName],
      reason: `expected ${resolveTypeName(def)}, got ${resolveActualType(value)}`,
    },
  ];
}

function runShapeCheck(
  obj: Record<string, unknown>,
...
export function objectGuard<T>(shape: Record<string, FieldDef>): Guard<unknown, T> {
  return guard<T>((arg) => {
    if (!isRecord(arg)) return { ok: false, failures: [{ path: [], reason: 'expected object' }] };
...
export function literal<T extends string | number | boolean>(value: T): Guard<unknown, T> {
  return guard<T>((arg) => {
    ...
    return {
      ok: false,
      failures: [
        { path: [], reason: `expected ${JSON.stringify(value)}, got ${JSON.stringify(arg)}` },
      ],
    };
  });
}
...
export function typeIs<A extends NonGuardTypeArg, T>(arg: T, typeArg: A): arg is T & Resolve<A> {
  if (typeof typeArg === 'string') return typeof arg === typeArg;

  const primitiveType = primitiveConstructors.get(typeArg);
  if (primitiveType !== undefined) return typeof arg === primitiveType;

  if (!isConstructorType(typeArg) || !isInstanceable(arg)) return false;
  return matchesConstructor(arg, typeArg);
}

export const cast = <A extends TypeArg>(arg: unknown, typeArg: A): Resolve<A> => {
  if (isGuardType(typeArg)) {
    const result = typeArg[GUARD_CHECK](arg);
    if (result.ok) return result.value as Resolve<A>;
    throw createNiceError(
      'Cast failed',
      ...result.failures.map((f): ErrorArg => [f.path.join('.') || 'value', f.reason]),
    );
  }

  if (typeIs(arg, typeArg)) {
    return arg as Resolve<A>;
  }
  throw createNiceError('Cast failed', [
    'value',
    `expected ${resolveTypeName(typeArg)}, got ${resolveActualType(arg)}`,
  ]);
};
